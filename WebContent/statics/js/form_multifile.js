...
function fncFileAddAndEvent() {
	var outHtml = '<input name="file" id="file" type="file"  multiple="multiple"  style="display:none;" />';
	$("#file").remove();
	...
	$("#file").on( "change" , function() {
		if (jQuery(this).val() != '') {
			...
			console.log("file length:"+ $("#file").files);
...
function selectFile(obj) {
	...
	//if (fileExt == 'JPG' || fileExt == 'PNG' || fileExt == 'GIF') {
	  ...
	  $("#page-wrapper").loadingView({'state':true});

	  	var totalCount = obj.files.length;
		for  (var i=0; i < obj.files.length; i++) {
			...
			$.ajax({
				...
				error : function(request, status, error ) {   // 오류가 발생했을 때 호출된다.
					...
					alert("잘못된 파일을 업로드하였습니다. : " + fileName);
				},
				...
				complete : function () {   // 정상이든 비정상인든 실행이 완료될 경우 실행될 함수
					...
					if (totalCount == 0) {
						$("#page-wrapper").loadingView({'state':false});
					}
