...
<%@ include file="./LNB.jsp" %>
								...
								<div class="map-layer-wrap">
									<div class="map-layer">
										<div class="map-info" style="z-index:2;">
											...
											<li id="illumination_panel_slope">수직 일사량: - w/㎡</li>
										</div>
										<div class="balloon-area" style="position: absolute; top: 10px; left: 10px"></div>
										<!-- <div class="balloon-area" style="position: absolute; top: 10px; left: 10px"><div class="balloon">201동</div></div> -->
									</div>
								</div>
 ...
 <script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=36d5b7cc4e118b280950b289f51126b6&libraries=services,clusterer,drawing"></script>
		...
		if (datas) {
			...
			for (i=0; i<datas.length; i++){
				...
				var chart_html = '<div class="col-md-6">';
				 ...
				 chart_html += '		<div><span>'+datas[i].info.name+'</span><span>'+$.number(datas[i].info.current_output,3)+' kW / '+$.number(datas[i].info.capacity,2)+' kW</span></div>'; // 2
				 chart_html += '	</div>';
				 chart_html += '	<canvas id="linechart'+i+'" width="*" height="100"></canvas>';
				 chart_html += '	<div class="totle-kwh v1">';
				 chart_html += '		<div><span>순간발전량</span><span><strong>'+$.number(datas[i].info.current_output,3)+' </strong>kW</span></div>';	//2
				 chart_html += '		<div><span>누적발전량</span><span><strong>'+$.number(datas[i].info.accu_energy,3)+' </strong>kW</span></div>'; //0
				 chart_html += '	</div>';
				 chart_html += '</div>';
				 chart_html += '</div>';
	...
	function displayChartData(datas){
			...
			for (i=0; i<23; i++){
				...
				var val_today ='0';
				 ...
				 for(ii=0; ii<datas.today.lastyear.length; ii++){
					...
					if ( hour == regStr[0]){
						val_lastyear = datas.today.lastyear[ii].sum_generate;
						break;
					} 
				}
				...
				today_lastyear.push(val_lastyear);
			 	...
			 	for(ii=0; ii<datas.today.yesterday.length; ii++){
					...
					if ( hour == regStr[0]){
						val_yesterday = datas.today.yesterday[ii].sum_generate;
						break;
					} 		
				}
				...
				for(ii=0; ii<datas.today.today.length; ii++){
					...
					if ( hour == regStr[0]){
						val_today = datas.today.today[ii].sum_generate;
						break;
					} 
				}
		...
		/* for(i=0; i<datas.today.lastyear.length; i++){
		
		
			
			if (datas.today.lastyear.length >=datas.today.yesterday.length 
					&& datas.today.lastyear.length>=datas.today.today.length) {
				var regStr = datas.today.lastyear[i].reg_date.split(" ")[1].split(":");
				today_labels.push(regStr[0]+"시 " + regStr[1]+"분");
			}
			
			today_lastyear.push(datas.today.lastyear[i].sum_generate);
			
		}
		
		for(i=0; i<datas.today.yesterday.length; i++){
			
			if (datas.today.yesterday.length >datas.today.lastyear.length 
					&& datas.today.yesterday.length>=datas.today.today.length) {
				var regStr = datas.today.yesterday[i].reg_date.split(" ")[1].split(":");
				today_labels.push(regStr[0]+"시 " + regStr[1]+"분");
			}
						
			today_yst.push(datas.today.yesterday[i].sum_generate);
			
		}
		
		for(i=0; i<datas.today.lastyear.length; i++){
		
			if (datas.today.today.length > datas.today.lastyear.length 
					&& datas.today.today.length > datas.today.yesterday.length) {
				var regStr = datas.today.today[i].reg_date.split(" ")[1].split(":");
				today_labels.push(regStr[0]+"시 " + regStr[1]+"분");
			}
		
			today_today.push(datas.today.today[i].sum_generate);
		...
		if(datas.daily != undefined) {
			for (i=0; i<31 ; i++) {
				...
				for(ii=0; ii<datas.daily.lastyear.length; ii++){
					...
					var regStr = datas.daily.lastyear[ii].reg_date_day.split("-");
										
					if (day == regStr[2]){
						val_lastyear = datas.daily.lastyear[ii].sum_generate;
						break;
					}
				...
				for(ii=0; ii<datas.daily.lastmonth.length; ii++){
					//	console.log("reg_date_day:" + datas.daily.lastyear[i].reg_date_day);	
					var regStr = datas.daily.lastmonth[ii].reg_date_day.split("-");
										
					if (day == regStr[2]){
						val_lastmonth = datas.daily.lastmonth[ii].sum_generate;
						break;
					}
				...
				for(ii=0; ii<datas.daily.thismonth.length; ii++){
					//	console.log("reg_date_day:" + datas.daily.lastyear[i].reg_date_day);	
					var regStr = datas.daily.thismonth[ii].reg_date_day.split("-");
						
					if (day == regStr[2]){
						val_thismonth = datas.daily.thismonth[ii].sum_generate;
						break;
					}
		...
		if (datas.monthly != undefined) {	
			for(i=0; i<12;i++){
				...
				//	console.log("reg_date_month:" + datas.monthly.lastyear[i].reg_date_month);
					var regStr = datas.monthly.lastyear[ii].reg_date_month.split("-");
					//monthly_labels.push(regStr[1]+"월 ");
					
					if ( month == regStr[1]){
						val_lastyear = datas.monthly.lastyear[ii].sum_generate;
						break;
					}
				}
				...
				//	console.log("reg_date_month:" + datas.monthly.lastyear[i].reg_date_month);
					var regStr = datas.monthly.thisyear[ii].reg_date_month.split("-");
					//monthly_labels.push(regStr[1]+"월 ");
					
					if ( month == regStr[1]){
						val_thisyear = datas.monthly.thisyear[ii].sum_generate;
						break;
					}
				}
